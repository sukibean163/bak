...
def publish(stub, topic, message):
    message_payload = []
    message_payload.append(pubsub_pb2.PubsubMessage(data=message))
    req = pubsub_pb2.PublishRequest(topic=topic, messages=message_payload)
    try:
        resp = stub.Publish(req, TIMEOUT)
        for t in resp.message_ids:
...
def main():
    ...
    list_subscriptions(stub, 'projects/graphic-adviser-125922/topics/sevone-data')
    # Publish a test message
    publish(stub, 'projects/graphic-adviser-125922/topics/sevone-data', 'this is another test')

    # Read back all the unacknowledged messages
    sub_stub = create_sub_stub()
    pull(sub_stub, 'projects/graphic-adviser-125922/subscriptions/test-subscriber')
